  ...
  const fetchTelemetryData = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/telemetry/get-data/?table=omega&start_time=${new Date(startTime).toISOString()}&end_time=${new Date(endTime).toISOString()}`
      );
