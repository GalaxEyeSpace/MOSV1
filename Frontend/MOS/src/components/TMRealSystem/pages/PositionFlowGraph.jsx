...
const PositionFlowGraph = () => {
  const [graphData, setGraphData] = useState([]);

  const fetchTelemetryData = async () => {
    try {
      const endTime = Date.now(); // Current Time (T)
      const startTime = endTime - 1000; // T - 1 second window

      const response = await fetch(
        `http://localhost:8000/telemetry/get-data/?table=position&start_time=${new Date(startTime).toISOString()}&end_time=${new Date(endTime).toISOString()}`
      );
...
export default PositionFlowGraph;
